...
uint16_t ping_sensor(void){
        ...
        if(!(PINB & (1<<SIG_PIN)))
        {
            ...
            return 0xFFFF;                 // 0xFFFF return value to indicate no object was found
        }
...
int main(void)
{
    ...
    while(1)
    {
        ...
        // 0xFFFF is only returned when an objectis NOT detected
        if (distance != 0xFFFF)
        {
            sprintf(serial_string,"%6u cm\n",distance); // Forms a serial string through sprintf()to
                                                        // allow the changing distance value to be
                                                        // printed
          
            serial0_print_string(serial_string); // Prints serial stirng to terminal to show user
